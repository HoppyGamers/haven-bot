...
const RPG_ENABLED = (process.env.RPG_ENABLED || 'false').toLowerCase() === 'true';
...
bot.on('command', async (data) => {
  ...
  switch (command) {
    ...
    case 'help': {
      ...
      const rpgSection = RPG_ENABLED && AGENT_ENABLED
        ? `\n\n**RPG System:**\n` +
          `\`${agentName} rpg help\` - RPG commands and how to play\n` +
          `\`${agentName} rpg setup <name> [system]\` - Start a campaign in this channel\n` +
          `*(Systems: dnd5e, starwars, scifi, horror)*`
        : '';
...
**Fun:**
\`/ping\` - Test the bot
\`/help\` - This message${agentSection}${rpgSection}
      `.trim();
    ...
    default: {
      ...
      if (customCmd) {
        ...
        const unknownMsg = await channelBot.sendMessage(
          `❓ Unknown command: \`/${command}\`. Type \`/help\` for available commands.`
        );
        const helpSecs = parseInt(process.env.HELP_DELETE_SECONDS || '0', 10);
        if (helpSecs > 0 && unknownMsg?.message_id) {
          setTimeout(() => channelBot.deleteMessage(unknownMsg.message_id).catch(() => {}), helpSecs * 1000);
        }
      }
    }
  }
...
async function main() {
  ...
  // Init RPG database if enabled
  if (RPG_ENABLED) {
    const { initRpgDatabase } = require('./rpg/database');
    initRpgDatabase();
    console.log('⚔️ RPG system enabled');
  }
